...
- (void)setupXGToolbar {
    if ((!_XGtoolbar)  && (!self.needHidenBar)) {
        CGFloat height = 44;
        CGFloat topH = 0;
        if (iPhoneX ||iPhoneXR||iPhoneXS ) {
           height = 44 + 34;
           topH = 64;
        }
        _XGtoolbar = [[LGPhotoPickerCustomToolBarView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds) - height-topH, CGRectGetWidth(self.view.bounds), height)];
