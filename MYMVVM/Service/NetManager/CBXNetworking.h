...
// 项目打包上线都不会打印日志，因此可放心。
#ifdef DEBUG
#define CBXAppLog(s, ... ) NSLog( @"[%@ in line %d] ===============>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define CBXAppLog(s, ... )
#endif
 ...
 *
 */
typedef void (^CBXDownloadProgress)(int64_t bytesRead,
                                    int64_t totalBytesRead);

typedef CBXDownloadProgress CBXGetProgress;
typedef CBXDownloadProgress CBXPostProgress;
 ...
 */
typedef void (^CBXUploadProgress)(int64_t bytesWritten,
                                  int64_t totalBytesWritten);

typedef NS_ENUM(NSUInteger, CBXResponseType) {
  kCBXResponseTypeJSON = 1, // 默认
  kCBXResponseTypeXML  = 2, // XML
  // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
  kCBXResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, CBXRequestType) {
  kCBXRequestTypeJSON = 1, // 默认
  kCBXRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, CBXNetworkStatus) {
    kCBXNetworkStatusUnknown          = -1,//未知网络
    kCBXNetworkStatusNotReachable     = 0,//网络无连接
    kCBXNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kCBXNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
// 且处理，请转换成对应的子类类型
typedef NSURLSessionTask CBXURLSessionTask;
typedef void(^CBXResponseSuccess)(NSURLSessionDataTask * task,id response);
typedef void(^CBXResponseFail)(NSURLSessionDataTask * task, NSError *error);
 ...
 *  @note 这里只提供公共api
 */
@interface CBXNetworking : NSObject
 ...
 *    @author 黄仪标
 *
 *    取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的CBXURLSessionTask对象，
